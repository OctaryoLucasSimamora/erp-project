...
<div class="card shadow">
    <div class="card-header">
        <h5 class="mb-0">Edit Request for Quotation</h5>
    </div>
    
    <div class="card-body">
        <form action="{{ route('purchase.rfq.update', $rfq->id) }}" method="POST" id="rfqForm">
            @csrf
            @method('PUT')
                        ...
                        <select name="vendor_id" class="form-control @error('vendor_id') is-invalid @enderror" required>
                            ...
                            @foreach($vendors as $vendor)
                                <option value="{{ $vendor->id }}" 
                                        {{ old('vendor_id', $rfq->vendor_id) == $vendor->id ? 'selected' : '' }}>
                                    {{ $vendor->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="col-md-3">
                    <div class="mb-3">
                        ...
                        <input type="date" name="deadline" 
                               ...
                               value="{{ old('deadline', $rfq->deadline ? date('Y-m-d', strtotime($rfq->deadline)) : '') }}">
                        @error('deadline')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="col-md-3">
                    <div class="mb-3">
                        ...
                        <input type="date" name="arrival_date" 
                               ...
                               value="{{ old('arrival_date', $rfq->arrival_date ? date('Y-m-d', strtotime($rfq->arrival_date)) : '') }}">
                        @error('arrival_date')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
            ...
            <div class="mb-3">
                ...
                <textarea name="notes" class="form-control @error('notes') is-invalid @enderror" 
                          rows="3">{{ old('notes', $rfq->notes) }}</textarea>
                @error('notes')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
...
<script>
let productLineCount = {{ count($rfq->lines) }};
