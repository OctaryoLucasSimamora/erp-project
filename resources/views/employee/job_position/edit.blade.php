...
<div class="card shadow">
    ...
    <div class="card-body">
        <form action="{{ route('employee.job_position.update', $jobPosition->id) }}" method="POST">
            @csrf
            @method('PUT')
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="text" name="position" 
                               ...
                               value="{{ old('position', $jobPosition->position) }}" required>
                        @error('position')
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="text" name="job_location" 
                               ...
                               value="{{ old('job_location', $jobPosition->job_location) }}">
                        @error('job_location')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                        ...
                        <select name="company" class="form-control @error('company') is-invalid @enderror" required>
                            ...
                            @foreach($companies as $company)
                                <option value="{{ $company }}" 
                                        {{ old('company', $jobPosition->company) == $company ? 'selected' : '' }}>
                                    {{ $company }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="number" name="expected_new_employees" 
                               ...
                               value="{{ old('expected_new_employees', $jobPosition->expected_new_employees) }}">
                ...
                <select name="department_name" class="form-control @error('department_name') is-invalid @enderror" required>
                    ...
                    @foreach($departments as $dept)
                        <option value="{{ $dept->department_name }}" 
                                {{ old('department_name', $jobPosition->department_name) == $dept->department_name ? 'selected' : '' }}>
            ...
            <div class="mb-3">
                ...
                <textarea name="job_description" class="form-control @error('job_description') is-invalid @enderror" 
                          rows="3">{{ old('job_description', $jobPosition->job_description) }}</textarea>
            ...
            <div class="d-flex justify-content-between">
                ...
                <a href="{{ route('employee.job_position.index') }}" class="btn btn-secondary">Cancel</a>
            </div>
        </form>
    </div>
</div>
